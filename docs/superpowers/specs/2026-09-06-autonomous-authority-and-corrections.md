...
`json_string` is exactly a JSON string token with standard JSON escapes, including `\"`, `\\` and Unicode escapes; decode it once, reject invalid escapes/unpaired surrogates, empty or whitespace-only values and decoded control characters. `bare_target` is the entire trimmed remainder, nonempty and containing no quote, backslash, comma, semicolon, CR/LF, second command introducer (`translate`, `qualify`, `that memory`, `this memory`, `this recollection` at word boundaries), or standalone ASCII-case-insensitive `and`/`or` word. Those restrictions intentionally make multi-target unquoted commands tentative; quote a literal rendering containing those characters/words. Periods in a bare target are **part of the rendering**, never silently stripped. No optional trailing sentence punctuation is allowed on rendering/qualification commands after a quoted token. Two commands in one event are not partially applied. Parse ambiguity, unsupported forms/languages, forbidden bare-target characters and malformed quoted strings yield `Tentative(AmbiguousIntent)`; they preserve the signal and mutate no rule/claim. Invalid structured transport fields remain `InvalidRequest`.
