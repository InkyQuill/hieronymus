...
The authority order during the migration is:

1. accepted ADRs and project instructions;
2. explicit compatibility specifications and versioned contract fixtures;
3. tests that implement those contracts;
4. current Python behavior where no higher-level decision changes it;
5. `docs/rust-migration-proposal/` as design input only.
...
4. rehearse upgrade and rollback on production-shaped copies;
5. switch managed installation to Rust only after every release gate passes;
6. retain the last Python release and database backup as the rollback path for
   the documented rollback window.
...
- rollback instructions are exercised against a release candidate backup.
