...
Package replacement snapshots authoritative native registration state before helper retirement or daemon stop, for both `update` and standalone bootstrap. Native rollback restores records and manager state together; it never writes an old task record over a still-suppressed task. Package journals remain until restoration and any explicitly authorized prior-active restart finish. On macOS, a loaded stable tray registration stays loaded while its helper retires. A formerly active Headless daemon's RunAtLoad reload is deferred until guarded restart after root ownership release and a durable Quit check. An originally loaded idle Headless agent refuses replacement before mutation because restoring its loaded state would start it. Idle Desktop agents can be restored without startup (RunAtLoad=false); unexpectedly lost tray registration refuses restoration instead of silently launching it.

The updater runs candidate `doctor --skip-registration --json` for independent payload/config checks. Its report explicitly identifies scope `payload-config-without-registration`; it is not a full doctor result. The parent separately validates daemon and tray registration while holding the existing lifecycle operation. Normal doctor retains its complete check set and reports scope `full`. Offline acceptance still rejects service/semantic warnings; only the documented uninitialized configuration-root warning is allowed after verified payload acquisition.

Windows helper execution resolves `selected-version.json` before finding the versioned helper, while `--binary` retains the stable launcher endpoint. Session and launch locks use private native creation and validate the very handle being locked. Windows coordination handles share read/write access for live lock probes, while secret-bearing readers retain their stricter sharing policy.
