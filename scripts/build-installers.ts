...
export const INSTALLERS = [
  ...
  "Install-Hieronymus.ps1",
] as const;
const template = (name: string) =>
  readFileSync(new URL(`./setup/${name}`, import.meta.url), "utf8");
function heredoc(path: string, contents: string) {
  const marker = "HIERONYMUS_EMBEDDED_FILE_END";
  if (contents.split("\n").includes(marker))
    throw new Error("embedded delimiter collision");
...
export function renderInstallers(releases: readonly ReleaseV2[]) {
  ...
  if (
    !/^\d+\.\d+\.\d+$/.test(version) ||
    releases.some((r) => r.version !== version || r.channel !== "stable")
  )
    throw new Error("one stable release version required");
  const url = `https://github.com/InkyQuill/hieronymus/releases/download/v${version}`;
  const fill = (text: string, payload: string) =>
    text
      .replaceAll("@@VERSION@@", version)
      .replaceAll("@@RELEASE_URL@@", url)
...
export async function buildInstallers(directory: string, output: string) {
  ...
  for (const r of releases)
    for (const artifact of [r.platform, r.model]) {
      if ((await digest(join(directory, artifact.archive))) !== artifact.sha256)
        throw new Error(`release bytes mismatch: ${artifact.archive}`);
    }
  const files = renderInstallers(releases);
  mkdirSync(output, { recursive: true });
  for (const name of INSTALLERS)
    writeFileSync(join(output, name), files[name], { flag: "wx", mode: 0o755 });
  return [...INSTALLERS];
}
