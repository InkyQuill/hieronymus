...
use common::{
    mcp_headers, mcp_protocol, route_target, send_request, start_daemon_on_ephemeral_port,
    substitute_placeholders, PROTOCOL_REVISION,
};
use serde_json::{json, Value};

fn post_mcp(daemon: &hiero::daemon::Daemon, headers: &[(String, String)], body: &Value) -> common::RawResponse {
    send_request(
...
fn expected_headers_from_fixture(request: &Value, daemon: &hiero::daemon::Daemon) -> Vec<(String, String)> {
    ...
    for (name, value) in request["headers"].as_object().unwrap() {
        ...
        } else if value == "Bearer hieronymus-invalid-bearer" {
...
fn route_case_failures_reproduce_the_frozen_http_contract() {
    ...
    for failure in target["failures"].as_array().unwrap() {
        ...
        let response = send_request(port, &request["method"].as_str().unwrap().to_string(), &request["path"].as_str().unwrap().to_string(), &headers, &body);
        ...
        assert_eq!(
            response.status as u64, expected_status,
            "case {id}: body was {:?}",
            response.body()
        );
        assert_eq!(
...
fn route_case_successes_keep_the_frozen_envelope() {
    ...
    for success in target["successes"].as_array().unwrap() {
        ...
        let mut headers = expected_headers_from_fixture(&request, &daemon);
        // The fixture freezes the placeholder bearer; the daemon's credential
        // is per-installation, so replace it with the real one.
        for header in &mut headers {
            if header.0 == "Authorization" {
                header.1 = format!("Bearer {}", daemon.bearer().expose_secret());
            }
        }
        ...
        assert_eq!(
            response.body()["jsonrpc"], request["body"]["jsonrpc"],
            "case {id}"
        );
        ...
        match id {
            ...
            "tools-call" => {
                ...
                assert_eq!(response.body()["result"]["resultType"], json!("complete"));
                assert!(
                    response.body()["result"]["content"].as_array().unwrap().len() > 0,
...
fn session_and_event_headers_are_rejected() {
    ...
    for (header_name, header_value) in [
        ...
        let response = post_mcp(
            &daemon,
            &mcp_headers(&daemon, &[("Mcp-Method", "tools/list"), (header_name, header_value)]),
            &request,
        );
        assert_eq!(response.status, 400, "{header_name}: {:?}", response.body());
        assert_eq!(response.body()["error"]["code"], json!(-32602), "{header_name}");
        assert_eq!(
...
fn not_yet_ported_tool_returns_clean_jsonrpc_error() {
    ...
    assert!(!message.contains("Bearer"), "diagnostics must not leak credentials");
}
...
fn served_registry_equals_the_frozen_snapshot() {
    ...
    assert_eq!(served_names.len(), snapshot["derived_tool_count"].as_u64().unwrap() as usize);
    assert_eq!(PROTOCOL_REVISION, snapshot["protocol_revision"].as_str().unwrap());
}
