...
#[test]
fn run_manual_dreaming_runs_the_deterministic_seam_and_audits() {
    let fx = setup();
...
fn run_manual_dreaming_ignores_a_provider_key() {
    // The DTO only accepts `all`; a `provider` key is silently ignored and the
    // seam stays deterministic-only (D5's provider lanes are out of scope).
    let fx = setup();
    let out = run(&fx, "run_manual_dreaming", json!({ "provider": "openai" }));
    assert!(
        out.is_ok(),
        "an unknown key is ignored; the seam stays deterministic"
    );
