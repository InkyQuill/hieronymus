...
fn ws_admin_event_ids_restart_on_a_new_daemon_session() {
    // First daemon session: one run advances the hub past id 1. The
    // controller resolves configured lanes per run; wire them so the run
    // passes the fail-closed workflow gate.
    let (first, first_root, first_daemon) = start_daemon_with_browser_session();
    wire_enabled_workflows(first_root.path());
    ...
    let (second, second_root, _second_daemon) = start_daemon_with_browser_session();
    wire_enabled_workflows(second_root.path());
