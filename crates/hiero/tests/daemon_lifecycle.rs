...
use common::{route_target, send_request, start_daemon_on_ephemeral_port};
...
fn write_python_sentinel_database(root: &Path) {
    let connection =
        rusqlite::Connection::open(root.join("hieronymus.sqlite")).unwrap();
    for table in [
        "series",
        "task_sessions",
        "short_term_memories",
...
fn corrupt_database_fails_closed() {
    let root = tempfile::tempdir().unwrap();
    std::fs::write(root.path().join("hieronymus.sqlite"), b"definitely not sqlite").unwrap();
...
fn shutdown_route_authenticates_then_stops_and_cleans_discovery() {
    ...
    let wrong = send_request(
        ...
        "POST",
        "/shutdown",
        &[("Authorization".to_string(), "Bearer wrong-token".to_string())],
        b"",
    );
...
fn daemon_binary_publishes_discovery_and_shuts_down_gracefully() {
    ...
    std::io::BufRead::read_line(&mut std::io::BufReader::new(child.stdout.take().unwrap()), &mut stdout)
        .expect("daemon must print its ready line");
    assert!(stdout.contains("listening"), "unexpected ready line: {stdout}");
