//! Bounded native LaunchAgent/system-opener isolation. Parent owns lifecycle/registration guards.
...
fn call(
    ...
    // Child response follows gate release.
    result
}
fn line_until(
        ...
        match (&request.url, request.action) {
            (Some(url), None) => {
                ...
                let status = Command::new("/usr/bin/open")
                    ...
                    .stderr(Stdio::null())
                    .status()
                    .map_err(|_| "Native browser opener failed")?;
                if !status.success() {
                    return Err("Native browser opener failed".into());
                }
                Ok(Value::Null)
            }
            (None, Some(action)) => crate::service::macos::execute(
                &ServiceOptions {
...
mod tests {
    ...
    fn native_transport_timeout_before_commit_aborts_and_after_commit_retains_authority() {
        ...
        let fixture = temp.path().join("after");
        let source =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/macos-native-broker.rs");
        assert!(
            Command::new("rustc")
