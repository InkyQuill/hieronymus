...
use hiero::daemon::{run_foreground, DaemonOptions};
use hiero::stdio::{run_stdio_adapter, StdioOptions};
        ...
        match argument.as_str() {
            ...
            "--port" => {
                ...
                parsed.port = Some(
                    value
                        .parse()
                        .map_err(|_| format!("--port requires a valid port number, got {value}"))?,
                );
            }
    ...
    match parsed.command.as_deref() {
        ...
        Some("daemon") => {
            ...
            let options = DaemonOptions {
                ...
                port: parsed.port.unwrap_or_else(|| hiero::daemon::DEFAULT_PORT),
            };
