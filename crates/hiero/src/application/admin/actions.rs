...
pub fn run_action(
    ...
    match action {
        ...
        "run_manual_dreaming" => run_manual_dreaming_action(config, actor, args),
...
/// `run_manual_dreaming({all?})` — run the deterministic dreaming seam
/// synchronously (the same `DreamService::open(config,
/// DeterministicDreamProvider).run_all(...)` path the daemon's dedicated
/// `/api/admin/actions/run_manual_dreaming` route drives in the background),
/// and audit it. D5's configured provider lanes are out of scope.
fn run_manual_dreaming_action(
    ...
    let record = DreamService::open(config, DeterministicDreamProvider)
        .and_then(|service| service.run_all("admin", drain_all, false))
        .map_err(domain)?;
