...
fn learn_skill() -> String {
    workflow_skill(
        ...
        r#"During normal reading and translation, store significant observations with hieronymus_short_term_add or batch, using kind/text and individually scoped claims. A new claim has this shape: `{"text":"Alex speaks in clipped phrases.","concept_id":2,"applicability":{"series_id":1,"timeline_id":1,"volume_key":"Book I","chapter_key":"chapter-01","scope_predicates":[],"valid_from":1,"valid_until":null,"metadata_state":"Resolved","knowledge_gates":[{"viewpoint":"Narrator","known_from":1,"known_until":null}]}}`. `text` and `concept_id` are siblings of `applicability`; omit a concept instead of inventing one. Use actual resolved positions and viewpoints. Never invent chronology, a character, or `All`; unresolved context or a missing knowledge gate remains conservatively outside current truth. Separate reusable voice/relationship assertions from chapter-local events and give each only the scope supported by the source. Inspect returned claim IDs, revisions and warnings before relying on later recall. Preserve uncertainty and source evidence. No author label, approval queue or special Learn request is needed.
