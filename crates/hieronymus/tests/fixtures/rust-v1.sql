-- FROZEN SNAPSHOT: what a Rust schema-version 1 database looks like.
--
-- Captured verbatim from `migrations/global.sql` + `migrations/terminology.sql`
-- plus the version markers `db.rs` published for schema version 1. This file is
-- the upgrade suite's "old database" and MUST NOT be regenerated when the
-- supported schema version is bumped: a v1 fixture that silently followed the
-- current schema would stop testing the upgrade at all. New schema belongs in a
-- new numbered migration step instead (see `migrations/002-durable-work.sql`).

-- ---------------------------------------------------------------------------
-- migrations/global.sql (schema v1)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- migrations/terminology.sql (schema v1)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Schema-version markers, exactly as db.rs published them at version 1.
-- ---------------------------------------------------------------------------
-- Whitespace matters: these are the exact bytes `db.rs` emitted, so the
-- fixture's `sqlite_master` text matches a real version-1 database.
create table if not exists hieronymus_meta (
             schema_version integer not null unique
         );
         insert or ignore into hieronymus_meta (schema_version)
         values (1);
