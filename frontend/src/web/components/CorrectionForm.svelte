...
<section class="m-5 grid gap-4 rounded-md border border-default bg-surface p-5" aria-label="Correct a memory">
  ...
  {:else if selection}
    <fieldset class="grid gap-2"><legend class="mb-2 font-medium">Choose the statement to correct</legend>{#each selection.claims as item (item.claim_id)}<label class="flex gap-2"><input type="radio" name="correction-claim" value={item.claim_id} bind:group={claim} disabled={busy || applied} onchange={edited}/><span>{item.text}<small class="block text-secondary">{scope(item.applicability)}</small></span></label>{/each}</fieldset>
    {#if selection.claims.length === 0}<p>This memory has no separately recorded statements to correct.</p>{/if}
  {/if}
  {#if mode !== "invalidate"}<label>{mode === "rendering" ? "Correct rendering" : "Pointer or context for your agent"}<textarea class="mt-1 block w-full rounded border border-default bg-surface p-2" bind:value disabled={busy || applied} oninput={edited}></textarea></label>{/if}
