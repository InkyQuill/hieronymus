...
export type SourceChoice = { id: number; series_id: number; selected_text: string; context?: string; source_identity: string; start: number; chapter: string | null; rules: Rule[]; context_unresolved?: boolean };
...
export type Selection = { series_id: number; expected_revision: number; source_language: string; target_language: string | null; claims: Claim[]; source: null | { reference: Record<string, unknown>; selected_text: string; binding: { applicability: Applicability } }; rule: Rule | null };
